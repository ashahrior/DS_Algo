"""
@https://github.com/zulkarnine/ds_algo/blob/master/BinaryTreePrinter.py
...
class NodePrintData:
    def __init__(self, lines: list, root_position, root_len):
        ...
        self.max_width = 0 if len(lines) == 0 else max(
...
class BinaryTreePrinter:
    ...
    def __treeify(self, node):
        ...
        if left_node_data.max_width > 0:
            ...
            branch_len = left_node_data.max_width - \
                (left_node_data.root_position + left_node_data.root_len)
        ...
        first_line += val
        left_padding = "" if right_node_data.max_width == 0 else " " * \
            (len(val) + 1 + self.extra_padding)
        if right_node_data.max_width > 0:
            first_line += self.branch_line * \
                (right_node_data.root_position + self.extra_padding)
            second_line += " " * (right_node_data.root_position +
                                  len(val) + self.extra_padding) + self.right_node_line
